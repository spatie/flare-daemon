...
An upstream `429` pauses only the affected API key and telemetry type. The next payload after the pause tries upstream again. A `Retry-After` header sets the pause length. Otherwise, the length depends on why Flare sent the `429`:
...
- **Spike protection**: a project that is spiking temporarily accepts fewer errors per minute. The pause lasts 10 seconds.

A plan limit (`Plan limit exceeded.`) is a `403`, not a `429`.
