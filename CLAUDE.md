...
- 429 pauses only the affected (key, type), honoring Retry-After, otherwise 60 seconds for a quota (`x-{type}-quota-reached: 1`) and 10 seconds for a rate limit or spike protection. Normal items, including already queued ones, are dropped on pause; diagnostic requests bypass it.
